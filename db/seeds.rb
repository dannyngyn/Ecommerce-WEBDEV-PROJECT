...
fishy_csv.each do |f|
  ...
  if water_type&.valid?
    if raised_type&.valid?
      water_type.fishs.create(raised_type:,
                              fish_name:   f["fish_name"],
                              stock:       f["stock"],
                              size:        f["size"],
                              fish_cost:   f["fish_cost"])
    else
      logger.debug "Raised Type is invalid"
    end
  else
    logger.debug "Water Type is invalid"
  end
end

fish_count = Fish.count
logger.info "Created #{fish_count} Fish Records w/ CSV"
...
logger.info "Created #{province_count} Province Records w/ Nokogiri Web Scraping"

10.times do
  random_province = Province.order("RANDOM()").take
  next unless random_province&.valid?

  random_province.users.create(province_id: random_province,
...
logger.info "Created #{user_count} User Records w/ Faker"
